...
import com.fallenangel.linmea._linmea.data.firebase.TestWrapper;
...
import com.fallenangel.linmea._linmea.model.SimpleDictionary;
...
public class TestConfiguratorActivity extends AppCompatActivity implements View.OnClickListener, SeekBar.OnSeekBarChangeListener {
...
//
//        Observable
//                .just(loadDictionaryList(mDatabaseReference, mDictionaryList))
...
//                .subscribe();
        ...
        Observable
               // .just(loadDictionaryList(mDatabaseReference, mDictionaryList))
                .just(loadDictionaryList(mDatabaseReference, mDictionaryList), loadSavedTestData())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnCompleted(() -> implementUI())
                .subscribe();

//        implementUI();
//
//        loadSavedTestData();
//        loadDictionaryList(mDatabaseReference, mDictionaryList);
    }
...
//        sbWordRepetitionCounter.setOnSeekBarChangeListener(this);
    ...
    private String setTestingWordsDescription() {
        return "picked " + testingConfigurator.getPickedWordsSize() + " words of " + testingConfigurator.getDictSize();
    }
    ...
    private void chooseTestingWords(){
...
//                    }
    ...
    private List<Word> loadDictionaryWords(){
        String path = "custom_dictionary/" + getCurrentUserUID() + "/dictionaries/" + testingConfigurator.getDictionaryName();
        if (!mDictionaryList.isEmpty())
            mDictionaryList.clear();
        mDatabaseReference.child(path).orderByChild("word").addChildEventListener(new ChildEventListener() {
        ...
        return simpleDictionary.getDictionary();
    }
            ...
            case R.id.testing_type_seek_bar:
                switch (testingConfigurator.getTestType()){
                    ...
                    case 2:
                        if (seekBar.getProgress() == 0){
                            Toast.makeText(getApplicationContext(), "Nope", Toast.LENGTH_SHORT).show();
                            testingConfigurator.setCorrectWordsCounter(1);
                            upUI();
                        } else {
                            testingConfigurator.setCorrectWordsCounter(seekBar.getProgress());
                        }
                        break;
                }
                break;
